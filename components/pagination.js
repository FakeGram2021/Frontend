  ...
  <div className="flex flex-col items-center my-12">
    <div className="flex text-gray-700">
      <div className="flex h-8 font-medium ">
        {!first && (
          <LeftLine
            pageLink={handlePreviousPageChange.path}
            pageQuery={handlePreviousPageChange.query}
          />
        )}
        <div
        ...
        {!last && (
          <RightLine pageLink={handleNextPageChange.path}
                     pageQuery={handleNextPageChange.query}/>
        )}
      </div>
    </div>
  </div>
...
const LeftLine = ({pageLink, pageQuery}) => (
  <div className="h-8 w-8 mr-1 flex justify-center items-center  cursor-pointer">
    <Link href={{pathname: pageLink, query: pageQuery}}>
      <svg
...
const RightLine = ({pageLink, pageQuery}) => (
